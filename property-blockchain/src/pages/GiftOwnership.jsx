...
const GiftOwnership = () => {
    ...
    const fetchMyProperties = async () => {
      ...
      try {
        ...
        for (const req of allRequests) {
          // Check if status is Approved (3)
          if (Number(req.status) === 3) {
            try {
              const owner = await contract.ownerOf(req.id);
              if (owner.toLowerCase() === walletAddress.toLowerCase()) {
                let imageUrl = "https://images.unsplash.com/photo-1500382017468-9049fed747ef";
                let title = req.ownerName || `Property #${req.id}`;
                
                if (req.ipfsMetadata && req.ipfsMetadata.startsWith("http")) {
                  try {
                    const response = await fetch(req.ipfsMetadata);
                    const json = await response.json();
                    if (json.image) imageUrl = json.image;
                    if (json.name) title = json.name;
                  } catch (err) {
                    console.warn("Failed to fetch IPFS metadata for property:", req.id.toString(), err);
                  }
                }
                
                myAssets.push({
                  id: req.id.toString(),
                  nftTokenId: req.id.toString(),
                  title: title,
                  location: req.landLocation,
                  imageUrl: imageUrl,
                  rawRequest: req
                });
              }
            } catch (e) {
              console.error("Error checking owner for token:", req.id.toString(), e);
            }
          }
        }
  ...
  const handleTransfer = async (e) => {
    ...
    try {
      ...
      console.log("Initiating Blockchain Transfer for Token:", selectedProperty.id);
      
      // Step A: Blockchain Transaction (transferFrom)
      const tx = await contract.transferFrom(walletAddress, formData.recipientWallet, selectedProperty.id);
      await tx.wait();
      
      alert("🎉 Property NFT Transferred Successfully on Blockchain!");
