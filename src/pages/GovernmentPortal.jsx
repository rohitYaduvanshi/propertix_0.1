...
const GovernmentPortal = () => {
    ...
    const fetchPendingRequests = async () => {
        try {
            ...
            // ✅ Step 1: Proxy Array को Normal Array में बदलें
            const allRequests = Array.from(data);

            // ✅ Step 2: सिर्फ Pending Requests निकालें और डेटा को Safe बनाएं
            ...
            const detailedPending = await Promise.all(pending.map(async (req) => {
                ...
                try {
                    ...
                    return {
                        ...req,
                        requestId: safeId,
                        ownerName: req.ownerName || "Unknown",
                        khasraNumber: req.khasraNumber || "N/A",
                    ...
                    console.warn("Metadata error for ID:", safeId);
                    return { ...req, requestId: safeId, landArea: req.landArea?.toString() || "0" };
                }
            }));
    ...
    if (loading) return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center">
            ...
            <p className="text-blue-500 font-black tracking-widest uppercase text-[10px]">Loading Secure Portal...</p>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#050505] text-white pt-24 pb-20 px-6 font-sans">
            ...
            {selectedDoc && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl">
                    <div className="bg-zinc-950 border border-white/10 w-full max-w-5xl h-[85vh] rounded-[40px] relative flex flex-col">
                        <div className="p-6 border-b border-white/5 flex justify-between items-center">
                            <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500">Document Evidence</h3>
                            <button onClick={() => setSelectedDoc(null)} className="p-2 hover:bg-white/5 rounded-full text-zinc-500 hover:text-white"><X size={24} /></button>
                        </div>
            ...
            <div className="max-w-7xl mx-auto">
                <header className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div>
                        <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.5em] mb-2">Govt_Verification_Node</p>
                ...
                {pendingRequests.length === 0 ? (
                    ...
                    <div className="grid gap-8">
                        ...
                        {pendingRequests.map((req, idx) => (
                            <div key={req.requestId || idx} className="bg-zinc-950 border border-white/5 rounded-[50px] overflow-hidden hover:border-blue-500/30 transition-all duration-500">
                                <div className="p-8 md:p-12 flex flex-col xl:flex-row gap-12">
                                    <div className="flex-1 space-y-8">
                                        ...
                                        <div className="flex flex-wrap gap-10 border-t border-white/5 pt-8">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-[8px] font-black text-zinc-600 uppercase mb-1">Area</p>
                                    ...
                                    <div className="w-full xl:w-72 flex flex-col gap-3 justify-center">
                                        <button onClick={() => setSelectedDoc(req.realImage)} disabled={!req.realImage} className="w-full bg-zinc-900/40 hover:bg-blue-600/10 border border-white/5 p-5 rounded-3xl flex items-center justify-between group disabled:opacity-20 transition-all">
                                            <div className="flex items-center gap-4">
                                                <ImageIcon className="text-blue-500" size={20} />
                                                <span className="text-[10px] font-black uppercase tracking-widest">Plot Photo</span>
                                            </div>
                                            <Eye size={14} className="text-zinc-700 group-hover:text-blue-500" />
                                        </button>

                                        <button onClick={() => setSelectedDoc(req.realDoc)} disabled={!req.realDoc} className="w-full bg-zinc-900/40 hover:bg-blue-600/10 border border-white/5 p-5 rounded-3xl flex items-center justify-between group disabled:opacity-20 transition-all">
                                            <div className="flex items-center gap-4">
                                                <FileText className="text-blue-500" size={20} />
                                                <span className="text-[10px] font-black uppercase tracking-widest">Deed Paper</span>
                                            </div>
                                    ...
                                    <div className="w-full xl:w-64 flex flex-col justify-center gap-4 border-t xl:border-t-0 xl:border-l border-white/5 pt-8 xl:pt-0 xl:pl-10">
                                        ...
                                        >
                                            {actionLoading === req.requestId ? "Bonding..." : "Authorize & Bond"}
                                        </button>
                                    </div>
                                </div>
                            </div>
