      ...
      <div className="relative w-full max-w-6xl grid lg:grid-cols-2 gap-8 lg:gap-16 items-start">
        ...
        <div className="w-full bg-zinc-900/40 backdrop-blur-2xl border border-white/10 p-5 md:p-10 rounded-[32px] shadow-3xl order-1 lg:order-2">
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            <div className="space-y-3 p-4 bg-black/40 rounded-3xl border border-zinc-800/50">
              <label className="text-[10px] uppercase text-cyan-400 font-black tracking-widest">Location (India)</label>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                <input type="text" placeholder="State" className="bg-zinc-900/80 text-xs p-3 rounded-xl border border-zinc-700 outline-none" onChange={(e)=>setFormData({...formData, state: e.target.value})} />
                <input type="text" placeholder="District" className="bg-zinc-900/80 text-xs p-3 rounded-xl border border-zinc-700 outline-none" onChange={(e)=>setFormData({...formData, district: e.target.value})} />
                <input type="text" placeholder="Village" className="bg-zinc-900/80 text-xs p-3 rounded-xl border border-zinc-700 outline-none" onChange={(e)=>setFormData({...formData, village: e.target.value})} />
              </div>
            ...
            <textarea placeholder="Address & landmarks" className="w-full bg-black/40 border border-zinc-800 p-4 rounded-2xl text-xs h-20 resize-none outline-none focus:border-cyan-500" onChange={(e)=>setFormData({...formData, address: e.target.value})} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div onClick={()=>document.getElementById('img-up').click()} className={`border-2 border-dashed p-5 rounded-2xl text-center cursor-pointer transition-all ${images.length === 3 ? "border-emerald-500 bg-emerald-500/5" : "border-zinc-800 hover:border-cyan-500"}`}>
                ...
                <p className="text-[9px] font-black uppercase text-gray-500">{images.length === 3 ? "✅ 3 Images" : "📸 3 Photos"}</p>
              </div>
              <div onClick={()=>document.getElementById('doc-up').click()} className={`border-2 border-dashed p-5 rounded-2xl text-center cursor-pointer transition-all ${docFile ? "border-emerald-500 bg-emerald-500/5" : "border-zinc-800 hover:border-cyan-500"}`}>
                ...
                <p className="text-[9px] font-black uppercase text-gray-500">{docFile ? "✅ PDF Ready" : "📄 PDF Doc"}</p>
              </div>
            </div>
